...
export function CaseStudiesSection() {
  return (
    <section className="section-pad py-24 md:py-32 bg-white dark:bg-[#0A0A0A] relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-12">
        ...
        <div className="space-y-4 mb-8">
          {cases.map((cs, i) => (
            <div key={i} className="border border-gray-200 dark:border-white/[0.1] rounded-2xl overflow-hidden hover:border-gray-300 dark:hover:border-white/[0.2] transition-all duration-300">
              <div className="h-px bg-gradient-to-r from-[#027C88]/60 to-transparent" />
              <div className="p-8 md:p-10">

                {/* Badges */}
                <div className="flex flex-wrap gap-2 mb-5">
                  <span className="font-jakarta text-[11px] font-bold tracking-[0.15em] uppercase px-3 py-1.5 rounded-lg border border-[#027C88]/40 text-[#027C88]">
                    {cs.industryBadge}
                  </span>
                  <span className="font-jakarta text-[11px] font-bold tracking-[0.15em] uppercase px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/[0.15] text-gray-500 dark:text-white/70">
                    {cs.capBadge}
                  </span>
                </div>

                <h3 className="font-syne font-bold text-gray-900 dark:text-white text-xl md:text-2xl mb-7 leading-snug max-w-3xl">
                  {cs.headline}
                </h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-7">
                  <div>
                    <p className="font-jakarta text-[11px] font-bold text-gray-400 dark:text-white/60 tracking-[0.2em] uppercase mb-2">The Situation</p>
                    <p className="font-jakarta text-gray-700 dark:text-white/80 text-sm leading-relaxed">{cs.situation}</p>
                  </div>
                  <div>
                    <p className="font-jakarta text-[11px] font-bold text-gray-400 dark:text-white/60 tracking-[0.2em] uppercase mb-2">What Kynato Built</p>
                    <p className="font-jakarta text-gray-700 dark:text-white/80 text-sm leading-relaxed">{cs.built}</p>
                  </div>
                </div>

                {/* Before / After */}
                <div className="grid grid-cols-2 rounded-xl overflow-hidden border border-gray-200 dark:border-white/[0.1] mb-6">
                  <div className="bg-gray-50 dark:bg-white/[0.03] p-5 border-r border-gray-200 dark:border-white/[0.1]">
                    <p className="font-jakarta text-[11px] font-bold text-gray-400 dark:text-white/60 tracking-[0.2em] uppercase mb-3">Before</p>
                    {cs.before.map((item, j) => (
                      <div key={j} className="font-jakarta text-sm font-medium text-gray-600 dark:text-white/75 mb-2.5 flex items-start gap-2">
                        <span className="text-gray-300 dark:text-white/40 mt-0.5 shrink-0 font-bold">–</span>
                        {item}
                      </div>
                    ))}
                  </div>
                  <div className="bg-[#027C88]/[0.04] p-5">
                    <p className="font-jakarta text-[11px] font-bold text-[#027C88] tracking-[0.2em] uppercase mb-3">After</p>
                    {cs.after.map((item, j) => (
                      <div key={j} className="font-jakarta text-sm font-semibold text-gray-800 dark:text-white mb-2.5 flex items-start gap-2">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <span className="font-jakarta text-[12px] font-medium text-[#027C88]">
                    Timeline: {cs.timeline}
                  </span>
                  <Link
                    href={cs.href}
                    className="inline-flex items-center gap-1.5 font-jakarta text-sm font-semibold text-gray-600 dark:text-white/75 hover:text-gray-900 dark:hover:text-white link-underline transition-colors duration-200"
                  >
          ...
          {/* Coming next */}
          <div className="border border-gray-200 dark:border-white/[0.08] border-dashed rounded-2xl p-8 text-center">
            <p className="font-jakarta text-[11px] font-bold text-[#027C88] tracking-[0.25em] uppercase mb-3">Coming Next</p>
            <p className="font-syne text-gray-800 dark:text-white/85 text-lg mb-4">
              A system like one of these — built for your business.
            </p>
            <Link href="/contact" className="inline-flex items-center gap-1.5 font-jakarta text-sm font-semibold text-[#027C88] hover:text-gray-900 dark:hover:text-white transition-colors">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 mt-12">
          <Link
            ...
            className="group inline-flex items-center gap-2 px-7 h-12 bg-[#027C88] text-white rounded-xl font-jakarta text-sm font-semibold hover:bg-[#026070] transition-all duration-300"
          >
            View All Case Studies <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-0.5" />
          </Link>
          <Link href="/contact" className="font-jakarta text-sm font-medium text-gray-600 dark:text-white/70 hover:text-gray-900 dark:hover:text-white link-underline transition-colors duration-200">
